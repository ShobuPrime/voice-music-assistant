...
- **NTP jurisdiction** — three files, all pointing time sync at NIST (US Dept of Commerce) instead of Alibaba Cloud:
  1. `buildroot/package/ntp/ntpd.etc.default` — `NTPSERVERS_IP` drops upstream's `203.107.6.88` + `120.25.115.20` (both Aliyun Computing Co., Hangzhou CN, WHOIS netname `ALISOFT`; the first is `ntp.aliyun.com`) for `2610:20:6f15:15::27` (`time-d-g.nist.gov`) + `2610:20:6f97:97::4` (`time-d-wwv.nist.gov`), keeping Cloudflare + Google as IPv4 fallback. `NTPSERVERS_DNS` drops `pool.ntp.org`. **`NTPDATE_OPTS` drops `-4`** — upstream forces IPv4, which would silently ignore the IPv6 NIST servers.
  2. `…/trspk/rootfs/etc/ntp.conf` — same list for the long-running `ntpd`.
  3. `…/trspk/rootfs/usr/share/udhcpc/default.script.d/ntp-from-dhcp` — `NTP_FALLBACK` must match, because this hook **regenerates `/etc/ntp.conf` on every DHCP lease**; miss it and a renew silently restores upstream's list.

  Why this is an addition and not a preference tweak: `ntpdate.sh` passes the whole server list to a **single** `ntpdate` invocation, which queries every server and picks the best responder — argv order confers no precedence and no early exit. So the Alibaba hosts got UDP/123 on every boot even when Cloudflare answered instantly, and **even when DHCP Option 42 supplied a local server**. `pool.ntp.org` is dropped for a different reason: it is the NTP Pool Project's global volunteer round-robin, so it cannot be reasoned about by jurisdiction or enumerated in a firewall rule. Retire this delta if upstream ever removes the Aliyun IPs itself.
